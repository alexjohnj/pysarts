...
def execute_std(args):
    if args.original:
        path = os.path.join(config.UIFG_DIR, args.slave_date + '_' +
                            args.master_date + '.nc')
        ifg = insar.InSAR.from_netcdf(path)
        print('Original: {:.5f}'.format(ifg.data.std()))

    if args.resampled:
        path = os.path.join(config.SCRATCH_DIR, 'uifg_resampled',
                            args.slave_date + '_' + args.master_date + '.npy')
        ifg_data = np.load(path)
        print('Clipped/Interpolated: {:.5f}'.format(ifg_data.std()))

    if args.corrected:
        path = os.path.join(config.SCRATCH_DIR, 'corrected_ifg',
                            args.slave_date + '_' + args.master_date + '.nc')
        ifg = insar.InSAR.from_netcdf(path)
        print('Corrected: {:.5f}'.format(ifg.data.std()))
